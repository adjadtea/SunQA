...
class Phpass {
	...
	{
		$output = '$P$';
		$output .= $this->_itoa64[min($this->ci->config->item('iteration_count') +
	...
	{
		$count_log2 = min($this->ci->config->item('iteration_count') + 8, 24);
	...
	protected function gensalt_blowfish($input)
	{
		...
		$output = '$2a$';
		$output .= chr(ord('0') + $this->ci->config->item('iteration_count') / 10);
		$output .= chr(ord('0') + $this->ci->config->item('iteration_count') % 10);
		$output .= '$';
	...
	public function hash_password($password)
	{
		...
		if (CRYPT_BLOWFISH == 1 && !$this->ci->config->item('portable_hashes')) {
		...
		if (CRYPT_EXT_DES == 1 && !$this->ci->config->item('portable_hashes')) {
			if (strlen($random) < 3)
